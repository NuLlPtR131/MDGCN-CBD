...
adj_list, features, labels, idx_train, idx_val, idx_test = load_our_data(args.dataset, cuda=True)
model = get_model(args.model, features.size(1), labels.max().item()+1, adj_list, args.hidden, args.out, args.dropout, cuda=True)

starttime=time.time()
ROC, F1, PR = predict_model(model, file_name, features, adj_list, eval_type, node_matching)
endtime=time.time()
